    ...
    while running:
        try:
            if not audio_queue.empty():
                ...
                # 一意のファイル名を生成
                temp_wav = os.path.join(TEMP_DIR, f"audio_{int(time.time() * 1000)}.wav")
                ...
                try:
                        ...
                        for retry in range(3):  # 最大3回リトライ
                            try:
                                ...
                                if retry < 2:  # 最後のリトライでなければ待機
                                    ...
                                    print("API failed after 3 attempts, skipping this audio")
                                    break
                    ...
                    should_ignore = False
                    text_lower = text.lower().replace(" ", "").replace("　", "")  # 小文字化 + スペース削除
                    ...
                    # 追加の強力なフィルタリング
                    if any(keyword in text_lower for keyword in ["thankyou", "ありがとう"]) and \
                       any(keyword in text_lower for keyword in ["watching", "視聴", "しちょう"]):
                        should_ignore = True
                    ...
                    # 翻訳を別スレッドで並列実行して高速化
                    translation_thread = threading.Thread(target=translate_and_update, args=(text,))
...
Return only the translation. Do not add explanations, comments, or notes."""
    ...
    try:
        ...
        if len(sentences) > 1:
            ...
            if not any(explain_word in first_sentence for explain_word in ["意味", "解釈", "訳", "表現", "場合"]):
